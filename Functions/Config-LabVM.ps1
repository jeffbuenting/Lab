...
#    Restart-VM -VM $VM -Confirm:$False | Wait-Tools
    ...
    Try {
        ...
        $Result = Invoke-VMScript -vm $VM -GuestCredential $LocalAdmin -ScriptText 'Set-NetFirewallRule -DisplayGroup "File and Printer Sharing" -Enabled True -Profile Any' -ErrorAction Stop
    ...
    Try {
        ...
        $Result = Invoke-VMScript -vm $VM -GuestCredential $LocalAdmin -ScriptText $CMD -ErrorAction Stop
        Write-Verbose $Result
    }
    Catch {
        $ExceptionMessage = $_.Exception.Message
    ...
    Try {
        ...
        $Result = Invoke-VMScript -vm $VM -GuestCredential $LocalAdmin -ScriptText "Enable-PSRemoting -SkipNetworkProfileCheck" -ErrorAction Stop
        Write-Verbose $Result
    }
    Catch {
        $ExceptionMessage = $_.Exception.Message
    ...
    $Result = Invoke-VMScript -VM $VM -GuestCredential $LocalAdmin  -ScriptText "Set-DscLocalConfigurationManager -path c:\temp -force"
    Write-Verbose $Result
