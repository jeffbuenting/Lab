...
Write-Verbose "Looping through Container host builds"
foreach ( $Node in ( $ConfigData.AllNodes | where Nodename -ne '*' ) ) {
    Write-Verbose "----------------------"
    Write-Verbose "Building Container VM ... $($Node.Nodename)"
    

    # ----- Build new VM if one does not already exist


    New-LABVM -VMName $Node.NodeName `
        -ESXHost $ConfigData.NonNode.ESXHost `
        -Template $ConfigData.NonNode.VMTemplate `
        -OSCustomization $ConfigData.NonNode.OSCustomization `
        -ResourcePool $ConfigData.NonNode.ResourcePool `
        -VMSwitch $ConfigData.NonNode.Switch `
        -PortGroup $ConfigData.NonNode.Portgroup `
        -DataStore $ConfigData.NonNode.DataStore `
...
#        Write-Warning "Invoke-VMScript		Failed to authenticate with the guest operating system using Local Credentials."
#
#        Try {
#            Write-Verbose "Attempting Domain Credentials"
#            Invoke-VMScript -vm $VM -GuestCredential $DomainAdmin -ScriptText $CMD -ErrorAction Stop
#        }
#        Catch {
#            $ExceptionMessage = $_.Exception.Message
#            $ExceptionType = $_.Exception.GetType().Fullname
...
#    }
    ...
    Try {
        ...
        Copy-Item -Path "$PSScriptRoot\mof\$($Node.NodeName).mof" -Destination RemoteDrive:\temp\localhost.mof -ErrorAction Stop -Force
